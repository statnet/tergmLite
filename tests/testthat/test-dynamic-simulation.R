...
test_that("manual and tergmLite dynamic simulations produce identical results for an ergm", {
  ...
  # set some arbitrary, non-default control to ensure it gets propagated correctly
  control <- control.simulate.formula(MCMC.burnin = 54321, MCMC.prop = ~Strat(~race, pmat=pmat) + TNT)
  ...
  nws <- simulate(ff, basis = nws, coef = nw_coef, constraints = constraints, control = control, output = "network", dynamic = FALSE)
  ...
  set.seed(0)
  nwL <- as.networkLite(nw)
  netest_ergm <- netest(nwL, ff, target.stats = target_stats, coef.diss = diss_coefs, constraints = constraints, set.control.ergm = list(init.method = "MPLE"))
